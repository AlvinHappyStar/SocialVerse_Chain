...
Find below a list of `socialvd` commands added with the `x/vesting` module. You can obtain the full list by using the `socialvd -h` command.
...
```go
socialvd add-genesis-account ADDRESS_OR_KEY_NAME COIN... [flags]
```
...
```go
socialvd query vesting balances ADDRESS [flags]
```
...
```go
socialvd tx vesting create-clawback-vesting-account TO_ADDRESS [flags]
```
...
```go
socialvd tx vesting clawback ADDRESS [flags]
```
...
```go
socialvd tx vesting update-vesting-funder VESTING_ACCOUNT_ADDRESS NEW_FUNDER_ADDRESS --from=FUNDER_ADDRESS [flags]
```
