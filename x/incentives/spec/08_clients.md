...
Find below a list of `socialvd` commands added with the `x/incentives` module. You can obtain the full list by using the `socialvd -h` command.
...
```go
socialvd query incentives incentives [flags]
```
...
```go
socialvd query incentives incentive CONTRACT_ADDRESS [flags]
```
...
```bash
socialvd query incentives gas-meters CONTRACT_ADDRESS [flags]
```
...
```go
socialvd query incentives gas-meter CONTRACT_ADDRESS PARTICIPANT_ADDRESS [flags]
```
...
```bash
socialvd query incentives params [flags]
```
...
```bash
socialvd tx gov submit-proposal register-incentive CONTRACT_ADDRESS ALLOCATION EPOCHS [flags]
```
...
```bash
socialvd tx gov submit-proposal cancel-incentive CONTRACT_ADDRESS [flags]
```
