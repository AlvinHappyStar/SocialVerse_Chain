...
socialvd tx revenue register $CONTRACT $NONCE $WITHDRAWER \
...
```bash
# Check revenues
socialvd q revenue contract $CONTRACT
```
...
socialvd tx revenue update $CONTRACT $WITHDRAWER \
...
socialvd tx revenue cancel $CONTRACT \
--gas=700000 --gas-prices=10000pose \
--from=mm
```
