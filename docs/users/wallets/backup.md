...
```bash
$ socialvd keys add dev0-restored --recover
...
```bash
socialvd keys unsafe-export-eth-key dev0 > dev0.export
...
```bash
$ socialvd keys import dev0-imported ./dev0.export
Enter passphrase to decrypt your key:
```
...
```
$ socialvd keys unsafe-import-eth-key dev0-imported ./dev0.export
