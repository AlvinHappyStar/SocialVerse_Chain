...
```bash
socialvd init $moniker --chain-id $CHAIN_ID
```

Move genesis file to .socialvd/config folder

```bash
mv $HOME/genesis.json ~/.socialvd/config/
```
...
```bash
sed -i -e "s%^moniker *=.*%moniker = \"$moniker\"%; " $HOME/.socialvd/config/config.toml
sed -i -e "s%^indexer *=.*%indexer = \"null\"%; " $HOME/.socialvd/config/config.toml
sed -i -e "s%^persistent_peers *=.*%persistent_peers = \"$PEER\"%; " $HOME/.socialvd/config/config.toml
```
...
s|^(seeds[[:space:]]+=[[:space:]]+).*$|\1\"\"|" ~/.socialvd/config/config.toml
```

### Create socialvd service

```bash
echo "[Unit]
Description=socialvd Node
After=network.target
#
[Service]
User=$USER
Type=simple
ExecStart=$(which socialvd) start
Restart=on-failure
LimitNOFILE=65535
#
[Install]
WantedBy=multi-user.target" > $HOME/socialvd.service; sudo mv $HOME/socialvd.service /etc/systemd/system/
```

```bash
sudo systemctl enable socialvd.service && sudo systemctl daemon-reload
```

### Run socialvd

```bash
sytemctl start socialvd
```

### Check logs

```bash
journalctl -u socialvd -f
```
...
```bash
sed -i.bak -E "s|^(enable[[:space:]]+=[[:space:]]+).*$|\1false|" $HOME/.socialvd/config/config.toml
```
