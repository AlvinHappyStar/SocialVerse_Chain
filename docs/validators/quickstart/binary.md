...
`socialvd` is the all-in-one command-line interface. It supports wallet management, queries and transaction operations {synopsis}
...
After you have obtained the latest `socialvd` binary, run:
...
::: tip
You can also enable auto-completion with the `socialvd completion` command. For example, at the start of a bash session, run `. <(socialvd completion)`, and all `socialvd` subcommands will be auto-completed.
:::

### Config and data directory

By default, your config and data are stored in the folder located at the `~/.socialvd` directory.
...
To specify the `socialvd` config and data storage directory; you can update it using the global flag `--home <directory>`

### Configuring the Node

The Cosmos SDK automatically generates two configuration files inside `~/.socialvd/config`:
...
To set pruning, adjust the `pruning` parameter in the `~/.socialvd/config/app.toml` file.
...
We can view the default client config setting by using `socialvd config` command:
...
A list of commonly used flags of `socialvd` is listed below:

| Option              | Description                   | Type                                             | Default Value |
| ------------------- | ----------------------------- | ------------------------------------------------ | ------------- |
| `--chain-id`        | Full Chain ID                 | `string`                                         | `""`          |
| `--home`            | Directory for config and data | `string`                                         | `~/.socialvd`   |
| `--keyring-backend` | Select keyring's backend      | `{"os"|"file"|"kwallet"|"pass"|"test"|"memory"}` | `"os"`        |
| `--output`          | Output format                 | `string`                                         | "text"        |

## Command list

A list of commonly used `socialvd` commands. You can obtain the full list by using the `socialvd -h` command.
